...
export default function DealerButton({ seatNumber, className }: DealerButtonProps) {
  ...
  return (
    <div
      className={cn(
        "absolute w-8 h-8 md:w-10 md:h-10 bg-card rounded-full shadow-lg border-2 border-chart-4 flex items-center justify-center z-40 transition-all duration-500",
        positionClasses[seatNumber],
        className,
      )}
    >
      <div className="relative">
        <span className="text-base md:text-lg font-bold text-foreground">D</span>
      </div>
    </div>
  )
}
